...
int syno_cpld_hdd_enable_write(int adapter, int address, int index, int val)
{
    int iRet = -1;
    union i2c_smbus_data data;
    struct i2c_adapter *pAdapter = NULL;
    ...
    if (val != ((data.byte >> (index - 1)) & 1)) {
        data.byte ^= (val << (index - 1));
        iRet = i2c_smbus_xfer(pAdapter, address, 0,
            I2C_SMBUS_WRITE, HDDBP_CPLD_ENABLE,
            I2C_SMBUS_BYTE_DATA, &data);
        if (iRet < 0) {
            goto END;
        }
...
void syno_smbus_hdd_powerctl_init(void){
	if(0 == strncmp(gSynoSmbusHddType, "tca9555", strlen("tca9555"))){
		...
		SynoSmbusHddPowerCtl.syno_smbus_hdd_enable_write=syno_cpld_hdd_enable_write;
